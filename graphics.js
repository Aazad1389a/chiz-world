...
function normalizeQuality(value) {
    if (typeof value === "string" && GRAPHICS_PROFILES[value]) {
        return value;
    }
...
/**
 * Resolve either:
 * - an actual HTMLCanvasElement
 * - a container containing a canvas
 * - a normal HTML container where a canvas should be created
 */
function resolveCanvas(target) {
    if (typeof document === "undefined") {
        throw new Error("WebGL requires a browser document.");
    }
    ...
    throw new Error(
        "createGameRenderer: a canvas or HTML container element is required."
    );
}
...
export function detectGraphicsCapabilities(renderer = null) {
    ...
    return {
        ...
        precision:
            capabilities.precision ?? "unknown"
    };
}

export function getRecommendedAnisotropy(
    renderer,
    ...
    const profile =
        GRAPHICS_PROFILES[normalizeQuality(quality)];
    ...
    return Math.max(
        1,
        Math.min(profile.textureAnisotropy, max)
    );
}
...
export function configureTexture(
    ...
    texture.anisotropy =
        getRecommendedAnisotropy(renderer, quality);
...
export function getGraphicsDiagnostics(
    ...
    const capabilities =
        detectGraphicsCapabilities(renderer);
 ...
 *
 * IMPORTANT:
 * This function now accepts either an actual canvas
 * OR a normal HTML container such as #game-canvas.
 */
export function createGameRenderer(
    ...
    if (!canvasOrContainer) {
        throw new Error(
            "createGameRenderer: canvas element or container was not found."
        );
    }

    const canvas =
        resolveCanvas(canvasOrContainer);

    if (!(canvas instanceof HTMLCanvasElement)) {
        throw new Error(
            "createGameRenderer: failed to create a valid HTMLCanvasElement."
        );
    }
    ...
    const profile =
        GRAPHICS_PROFILES[selectedQuality];
